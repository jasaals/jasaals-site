...
function buildGalleryCarousel(allPhotos) {
    const recent24 = allPhotos.slice(-24);
    const row1Photos = recent24.slice(-12);
    const row2Photos = recent24.length > 12 ? recent24.slice(0, recent24.length - 12) : [];
  ...
  const row2HTML = row2Photos.length > 0 ? `
      <div class="gallery-carousel-row">
            <button class="carousel-arrow" id="row2-prev" aria-label="Previous">&#8592;</button>
                  <div class="carousel-track-wrapper">
                          <div class="carousel-track" id="row2-track">
                                    ${row2Photos.map(makeTile).join('')}
                                            </div>
                                                  </div>
                                                        <button class="carousel-arrow" id="row2-next" aria-label="Next">&#8594;</button>
                                                            </div>
                                                                <div class="carousel-dots" id="row2-dots"></div>
                                                                  ` : '';

  galleryEl.innerHTML = `
      <div class="gallery-carousel-wrapper">
            <div class="gallery-carousel-row">
                    <button class="carousel-arrow" id="row1-prev" aria-label="Previous">&#8592;</button>
                            <div class="carousel-track-wrapper">
                                      <div class="carousel-track" id="row1-track">
                                                  ${row1Photos.map(makeTile).join('')}
                                                            </div>
                                                                    </div>
                                                                            <button class="carousel-arrow" id="row1-next" aria-label="Next">&#8594;</button>
                                                                                  </div>
                                                                                        <div class="carousel-dots" id="row1-dots"></div>
                                                                                              ${row2HTML}
                                                                                                  </div>
                                                                                                    `;

  initCarousel(
        document.getElementById('row1-track'),
        document.getElementById('row1-prev'),
        document.getElementById('row1-next'),
        document.getElementById('row1-dots'),
        row1Photos
      );

  if (row2Photos.length > 0) {
        initCarousel(
                document.getElementById('row2-track'),
                document.getElementById('row2-prev'),
                document.getElementById('row2-next'),
                document.getElementById('row2-dots'),
                row2Photos
              );
  }
}
